...
\begin{algorithm}
	...
	\begin{algorithmic}
		...
		\IF{$\exists C\in X$ s.t. $res(C)>= |C_{merged}|-|C|$}
		\STATE move all nodes from $C_{merged}$ to the server of $C$, freeing reservations and capacity
		\STATE $res(C_{merged})\gets res(C)-(|C_{merged}|-|C|)$
		\ELSE
		\STATE $target\gets$ $server$ s.t. $cap(server)\geq \min\{k, (1+\epsilon)\cdot|C_{merged}|\}$
		\STATE move all nodes from $C_{merged}$ to $target$, freeing reservations and capacity
		\STATE $cap(target)\gets cap(target)-\min\{k, (1+\epsilon)\cdot|C_{merged}|\}$
		\STATE $res(C_{merged})\gets\min\{k-|C_{merged}|, \epsilon\cdot|C_{merged}|\}$
		\ENDIF
	\end{algorithmic}
\end{algorithm}
