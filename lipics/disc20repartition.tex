...
We consider the problem of maintaining a partitioning of a dynamic graph 
consisting of $n=k\cdot \ell$ nodes that communicate with each other 
into $\ell$ clusters, each of size $k$, while minimizing both the cost due to communication and due to node migrations. 
...
problem. The authors propose a $O(k \log k)$-competitive algorithm 
with augmentation $2+\epsilon$ for any $\epsilon>1/k$ that takes exponential time. 
They also show a lower bound of $k-1$ for the competitive ratio of any online algorithm 
