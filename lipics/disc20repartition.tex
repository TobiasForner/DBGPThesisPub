...
In \cref{sec:related_work} we discuss previous work related to the DBGP problem. After that we describe two algorithmic ideas, denoted by \adjDel{} and \coreDel{}, which are inspired by prior work (\cite{Avin2015, Avin2016}) and the respective implications and challenges for their competitive analysis. 
...
After that we empirically evaluate our algorithms by comparing their results with those of existing implementations in \cref{sec:evaluation}.
% We summarize the results of this thesis in \cref{sec:conclusion}.
Finally we describe avenues for future work in \cref{sec:future_work}.
...
\begin{lemma}
	...
	Given a mergeable set of components $S$ and a partitioning of $S$ into $g\geq2$ parts $S_1,...,S_g$. The weight between the parts $S_i$ is at most $(g-1)\cdot\alpha$ during the execution of \adjDel{} (in the version \adjDel{}).
\end{lemma}
...
We compare these algorithms with the static graph partitioning algorithm METIS\_PartGraphRecursive implemented in the METIS framework (\cite{Karypis1998, Karypis1998a}) which we will refer to from now on as \static{} available in the ParMetis framework. %Both frameworks are known to produce very good results and to be very fast.
...
%\cref{fig:all_runtimes_boxplot} shows a comparison of the running times 
...
the plots behave very similarly. While we can not prove this, we suspect that this may 
be due to the fact that the data used for the run shows little structure and thus leads
a large number of (costly) recomputation steps for our data structure. This is also supported by the fact that
\directDecomp{} is significantly faster for \fb{} as one can see in 
