...
By collocating frequently communicating nodes on the same server,
...
the online algorithm does not have prior knowledge of future communication requests. The goal is to strike a balance between the benefits
...
\begin{center}
	\begin{figure}	
		...
		\end{tikzpicture}
		\caption{Example graph illustrating the decomposition 
		described in Section \ref{implDetSection}.}
		\label{ex_graph}
	\end{figure}
\end{center}
...
\cref{fig:all_runtimes_boxplot} shows the results of all data sets for \adjDel{}, \directDecomp{} and \coreDel{}.  Here
...
\begin{figure}
	\begin{minipage}{0.48\linewidth}
		\begin{subfigure}[b]{\linewidth}
			\includegraphics*[height=7cm]{plots/plot_facebook_time_to_size}
			\caption{Relation of size to time for \fb{} for \adjDel{}}
		\end{subfigure}		
	\end{minipage}
	\begin{minipage}{0.48\linewidth}
		\begin{subfigure}[b]{\linewidth}
			\includegraphics*[height=7cm]{plots/plot_facebook_time_to_size_coreDel}
...
\end{minipage}
	...
	\begin{minipage}{0.48\linewidth}
		\begin{subfigure}{\linewidth}		
			\includegraphics*[height=6cm]{plots/plot_varying_alphas_mocfe_coreDel}
...
\cref{fig:alpha_influence} shows a plot of the running time of \adjDel{} and 
\coreDel{} for $\alpha\in\{1,2,4,8,16\}$. In order to improve the readability,
only requests which led to an update of the data structure are included. 
The results show that both algorithms are slower for $\alpha=1$ than for 
$\alpha\in\{2,4\}$.
This may be due to the fact that lower values of $\alpha$ lead the algorithms to 
update the tree structure more frequently.
Especially for $\alpha=1$ every inserted edge leads to a merge, as edges are only 
inserted between different components and $\alpha$ is both the cost for a node
 migration as well as the connectivity threshold for merging for \adjDel{} and \coreDel{}.
...
as the update times may increase. This effect increases the runtime of \coreDel{} more than that \adjDel{}
leading to significantly better runtime for \adjDel{} for $\alpha=16$.
