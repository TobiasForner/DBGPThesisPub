...
\stefan{Herr Forner: in figures und tabellen vermeidet man 
grosse zahlen, also statt 100000 eher 100k etc.}
...
\end{figure}
	...
	We evaluate the running time of \adjDel{} by 
	considering different traces 
	and alternative algorithms. 
\cref{fig:all_runtimes_boxplot} shows the results
for different traces and for \adjDel{} and \directDecomp{}. Here
...
We can see that for both algorithms \fb{} results in the highest
runtime and that \directDecomp{} performs significantly better for \fb{}.
We first compare the runtimes of both algorithms for the hpc data sets before we discuss the run times for \fb{} in greater detail.
\stefan{ich glaube runtime ist besser als run time}
...
\cref{fig:hpc_runtimes_boxplot_restricted} shows for run times of \adjDel{} 
and \directDecomp{} for the hpc traces. In the top of the figure 
we can see the 
distribution of the update times over the requests, 
while the total times are shown 
at the bottom. The figure shows that \adjDel{} is considerably faster for both hpc traces.
...
for handling a 
request, and the number of edges in the data structure for \adjDel{} for \fb{}. 
...
the influence of $\alpha$ on the running time of \adjDel{}.
...
and migration, we compare our algorithm with \static{}.
...
Note that for \fb{} the cost of \adjDel{} is actually lower than the cost of \static{}. For the hps traces
...
\static{} may not produce a perfectly 
